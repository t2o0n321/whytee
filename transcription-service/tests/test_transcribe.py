...
def test_captions_tier_is_used_when_available(monkeypatch):
    segs = [
        Segment(start=0.0, duration=2.0, text="hello"),
...
def test_falls_back_to_audio_when_no_captions(monkeypatch):
    ...
    monkeypatch.setattr(orchestrator.audio, "download_audio", lambda vid, d: "/tmp/x.wav")
    monkeypatch.setattr(orchestrator.audio, "split_audio", lambda p: ["chunk0"])
